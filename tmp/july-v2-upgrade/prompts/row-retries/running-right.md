Create Codex pet row `running-right` for `july-the-dog`: exactly 8 full-body frames in one horizontal strip on flat pure cyan #00FFFF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Preserve July exactly: tiny red Shiba Inu, curled tail, bright alert eyes, compact cute coding companion. Existing 8x9 atlas is approved and must be the visual identity reference.. Preserve silhouette, face, palette, material, proportions, markings, and props.
