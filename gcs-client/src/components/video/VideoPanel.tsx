...
export default function VideoPanel({ panelId }: VideoPanelProps) {
  const { config, uav1, uav2, isConfigured, yoloEnabled } = useGCSStore();
  const uavConfig = panelId === 1 ? uav1 : uav2;
  ...
  useEffect(() => {
    if (uavConfig?.streamPort) setUdpPort(parseInt(uavConfig.streamPort, 10) || udpPort);
    if (uavConfig?.jsonPort) setJsonPort(parseInt(uavConfig.jsonPort, 10) || jsonPort);
  }, [uavConfig]);
  ...
  const onMessage = useCallback((data: string) => {
    ...
    try {
      ...
      if (msg.type === "telemetry") {
        ...
        if (msg.detection) {
          setVideoStats({
            gsd: msg.lokasi_target?.gsd_x?.toFixed(3) || "--",
            dist: msg.lokasi_target?.distance_m?.toFixed(1) || "--",
  ...
  return (
    <div className="video-panel" id={`${panelId === 1 ? "left" : "right"}-panel`}>
      {/* Camera source selector matching index.html */}
      <div className="cam-selector">
        <select
          id={`cam-source-${panelId}`}
        ...
        {sourceMode === "udp" && (
          <input
            ...
            className="cam-select"
            value={udpPort}
            onChange={(e) => setUdpPort(parseInt(e.target.value, 10) || udpPort)}
            style={{ width: "70px", marginLeft: "5px" }}
            placeholder="Vid Port"
          />
        )}
      </div>
        ...
        {noSignal && (
          <div className="no-signal-overlay">
            <div className="no-signal-inner">
              ...
              <div style={{ color: "#555540", fontSize: 12, letterSpacing: "0.15em", marginTop: 8 }}>NO SIGNAL</div>
              <div style={{ color: "#555540", fontSize: 10 }}>{noSignalText}</div>
            </div>
          </div>
        )}
      ...
      <div className="video-stats">
        ...
        <div style={{ width: "100%", display: "flex", gap: 10 }}>
          <div>uav: <span className="stat-value" style={{ fontSize: "0.9em" }}>{videoStats.uavLoc}</span></div>
          <div>tgt: <span className="stat-value" style={{ fontSize: "0.9em" }}>{videoStats.tgtLoc}</span></div>
        </div>
      </div>
    </div>
  );
